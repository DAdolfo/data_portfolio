...
logger.info(f"There are {len(sessionids_with_null_userids)} sessions without user_ids.")
#Check if any user_id is null for a session that does have a user_id record in any row
null_users_check_list = events.select("session_id") \
        .where(
...
special_session_ids = [row.session_id for row in null_users_check_list]

#I considered filling nulls for "What if a session id has only half the user_id values attributed to that session as null?""
#Due to the complexity of accomplishing this in Spark I think, as a Data Engineer, I'd suggest to fix the problem 
#that gives us the event before putting this load in our pipeline, if the rate of events with that problem is significant

if special_session_ids:
    logger.info(f"{len(special_session_ids)} users detected for various sessions.")
    events = (events
        .filter(~(F.col("session_id").isin(special_session_ids)))          
    )

#Only proceed if we have sessions without user_ids
if sessionids_with_null_userids:
    logger.info("Sessions will be given a fake user id.")
    events.createOrReplaceTempView("events_table")
    query = """
    SELECT 
        COUNT(DISTINCT session_id) * 1.0 / COUNT(DISTINCT user_id) AS avg_sessions_per_user
    FROM 
        events_table
    WHERE 
        user_id IS NOT NULL;
